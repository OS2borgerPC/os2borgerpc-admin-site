...
def make_proxy_password(pc):
    ...
    if not os.path.isfile(settings.PROXY_HTPASSWD_FILE):
        cmd.append("-c")
    cmd.extend([settings.PROXY_HTPASSWD_FILE, pc.uid, proxy_password])
    subprocess.call(cmd)
...
def get_proxy_setup(pc_uid):
    ...
    try:
        entry = pc.configuration.entries.get(key="proxy_upstream_password")
    ...
    context = Context(
        {
            "allowed_hosts": pc.get_merged_config_list(
                "proxy_allowed_hosts", default=settings.DEFAULT_ALLOWED_PROXY_HOSTS
            ),
            "upstream_host": pc.get_config_value(
                "proxy_upstream_host", "bibos-admin.magenta-aps.dk"
            ),
            "upstream_port": pc.get_config_value("proxy_upstream_port", "80"),
            "login": pc.uid,
    ...
    if direct_hosts_str:
        if direct_hosts_str != "":
            context["direct_hosts"] = [s.strip() for s in direct_hosts_str.split(",")]
    else:
        context["direct_hosts"] = settings.DEFAULT_DIRECT_PROXY_HOSTS

    template = template_loader.get_template("proxyconf/squid-deb-proxy.conf")
