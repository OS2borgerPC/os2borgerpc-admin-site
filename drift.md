...
`CORE_SCRIPT_COMMIT_HASH` er det commit hash som hører til `CORE_SCRIPT_VERSION_TAG`. Når man har valgt sin version, skal man således finde det tilhørende commit hash, og angive det her. Commit-hash kan ses på github under commit-historik (det skal være det fule hash, og ikke den forkortede udgave), ellers, hvis man blot angiver version og udelader commit-hash, vil admin-site under opstart printe en fejl til loggen, hvor commit-hash også står i.

Dette er en sikkerhedsforanstaltning der gør, at der ikke kan laves om i scripts, når først de har fået et versions-tag (f.eks. v1.2.0). Hvis det ændrer sig, vil admin site opdage det, og nægte at starte, før de to (commit-hash og version) stemmer overens. Det er også her det forventede hash printes til loggen, som nævnt ovenover.

Et versions-tag bør aldrig ændre commit-hash. Det vil kun ske, hvis der laves om i et eksisterende release af scripts -  modificerede scripts udgives med et eksisterende versionsnummer, og overskriver derved det tidligere release. Dette opdages ikke, hvis commit-hash ikke også angives.
...
Dette betyder at scripts vil akkumulere over tid, hver gang man angivere en nyere version af scripts.\
Hvis man vil rydde op i (ældre releases af) scripts, skal man gøre det manuelt. Dette vil typisk gøre via SQL, ved at tilgå den kørende database-container. Det anbefales også at fjerne evt. scripts der ikke bruges fra mappen `/media/script_uploads` i den kørende container. Denne mappe indeholder selve scriptet, mens databasen indeholder metadata vedr. scriptet.\
...
Fjerner man alle scripts og genstarter, får man ryddet helt op, og kun den version man har angivet vil være installeret. Mappen `/media/script_uploads` (i docker-containeren) skal dog stadig opryddes manuelt.
